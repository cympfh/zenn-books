...
sed が扱うパターンは慣例として正規表現と呼ばれてはいるが, 実際には前方参照という機能拡張によって厳密には正規表現ではなく, もっとより強い何かになっている.
...
`z` ではない小文字アルファベットが一文字与えられる.
ABC 順でその次のアルファベットを出力せよ (`a` → `b`, `b` → `c`, ..., `y` → `z`).

`z` を除けば与えられる入力は 25 通り. 変換も 25 パターンだけ. 全列挙しよう.
...
素直に読むと $|S|=1$ なら 6 回, $|S|=2$ なら 3 回, $|S|=3$ なら 2 回繰り返せ, と書いてある.
しかしもっとズボラに次のようにしてよい.
...
各文字について, 出現したかどうかチェックして, 出現したらカウントする. 最後にカウントが 4 になってたらOK. 入力の文字列は自由にイジっても大丈夫そうなので, パターンラインの頭に `@` を追加することでカウントしよう. `@` という文字は, 入力に含まれないはずの文字から選んだだけで意味はない.
...
例えば sed には整数といったデータを扱う方法は無いので「二つの整数が与えられるのでその和を出力せよ」といった問題は普通には解けない.
...
二行の入力に関するマッチをしたいわけだが, sed は基本的には一行ずつ読んで, 一行に関するパターンマッチを行う.
そこで `N` コマンドは次の行を追加で読むことが出来る（改行文字が挟まることに注意）. これを使えば二行以上に渡るパターンマッチが出来るようになる.
...
注意として, `c` コマンドは続く文字列を改行まで読む.
`c` コマンドの次の行を一行にまとめて書いてはいけない.
...
どれかの `s` で置換が無事成功したとき, 続く他のパターンマッチングは実行されてほしくない.
そこで `t` コマンドは直前の置換が成功した場合にジャンプ (goto) が出来る. `:` でラベルを作りこのラベルを指定したジャンプが出来る.
...
この二つをいっぺんに解決するのがパターン全列挙だ. 入力の長さが最大で 10 ということは, 最長文字列の長さは 0 以上 10 以下の 11 通りしかないことがわかる. 全列挙できる.
